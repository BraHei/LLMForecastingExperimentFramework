...
def generate_time_series(max_kernels: int = 5, sequence_lenght: int = DEFAULT_LENGTH):
    ...
    """
    while True:
        X = np.linspace(0, 1, sequence_lenght)

        # Randomly select upto max_kernels kernels from the KERNEL_BANK
        selected_kernels = np.random.choice(
            make_kernel_bank(sequence_lenght), np.random.randint(1, max_kernels + 1), replace=True
        )
