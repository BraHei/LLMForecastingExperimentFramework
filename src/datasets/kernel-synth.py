...
def generate_time_series(max_kernels: int = 5, sequence_length: int = DEFAULT_LENGTH):
    ...
    while True:
        ...
        selected_kernels = np.random.choice(
            make_kernel_bank(sequence_length), np.random.randint(1, max_kernels + 1), replace=True
        )
